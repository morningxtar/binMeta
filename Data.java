...
public class Data implements Comparable<Data> {
    ...
    public Data(int n, double p) {
        try {
            if (n <= 0) throw new Exception("Specified size for random Data object is zero or even negative");
            if (p < 0.0 || p > 1.0)
                throw new Exception("Specified probability of 1's in random Data object should be contained in [0,1]");
            int N = 1 + (n - 1) / 8;
    ...
    public Data(Data D, int h) {
        try {
            ...
            if (h != 0) {
                ...
                if (h >= nb)
                    throw new Exception("The specified Hamming distance is larger than the total number of bits");
            }
            this.size = nb;
            this.data = new ArrayList<Byte>(D.numberOfBytes());
            for (byte b : D.data) {
    ...
    public Data(Data D1, Data D2, String op) {
        try {
            ...
            if (D1.numberOfBits() != D2.numberOfBits())
                throw new Exception("The two Data objects have different size (in terms of bits)");
    ...
    // Data constructor: it generates a new Data object consisting of n bits forming a given byte
    //                   the bit pointer is set to 0
    ...
    // Data constructor: it generates a new Data object consisting of n bits forming a given integer
    //                   the bit pointer is set to 0
    ...
    // Data constructor: it generates a new Data object consisting of n bits forming a given long integer
    //                   the bit pointer is set to 0
   ...
   private ArrayList<Byte> fillArrayListByte(Object objects, int size){
    ...
    public Data(char c, boolean isLetter) {
        try {
            ...
            if (isLetter) {
                if ((c < 'a' || c > 'z') && (c < 'A' || c > 'Z'))
                    throw new Exception("The char is not an alphabet letter");
    ...
    public int hammingDistanceTo(Data D) {
        try {
            ...
            if (this.numberOfBits() != D.numberOfBits())
                throw new Exception("Impossible to compute Hamming distance: number of bits differ");
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
    ...
    public boolean belongsToNeighbourOf(Data D, int h) {
        try {
            if (h < 0)
                throw new Exception("Impossible to verify neighbourhood of Data object: specified hamming distance is negative");
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
    ...
    public Data randomSelectInNeighbour(int l, int u) {
        try {
            if (l <= 0)
                throw new Exception("Impossible to verify neighbourhood of Data object: lower bound on hamming distance is nonpositive");
            if (u < 0)
                throw new Exception("Impossible to verify neighbourhood of Data object: upper bound on hamming distance is negative");
            if (l > u)
                throw new Exception("Impossible to verify neighbourhood of Data object: upper bound is greater than lower bound on hamming distance");
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
    ...
    public char charValue() {
        try {
            if (this.numberOfBits() != 8)
                throw new Exception("Impossible to convert in ASCII character: number of bits is not 8");
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
        }
    ...
    public char letterValue() {
        try {
            if (this.numberOfBits() > 5)
                throw new Exception("Impossible to convert in capital letter: too many bits (only 5 required)");
            int gap = this.intValue();
            if (gap > 25)
                throw new Exception("Impossible to convert in capital letter: no letter after 'Z' (bit code is " + gap + ">25)");
            return (char) ('A' + gap);
        } catch (Exception e) {
            e.printStackTrace();
            System.exit(1);
    ...
    public static void main(String[] main) {
        ...
        while (compt < nTest) {
            ...
            Data DD = new Data(n, bit);

            //random number of bit in D
            int bitIndex = 1 + R.nextInt(D.numberOfBits() - 1);

            //remove characters [, |, ] of toString's result
            String toStringWithoutSomeCharacters = D.toString().replaceAll("[\\[|\\]]", "");

            //test on method toString
            if (!(toStringWithoutSomeCharacters.charAt(bitIndex) + "").equals(D.getBit(bitIndex) + "")) testOK = false;

            //test on method hashCode
            if (D.hashCode() != DD.hashCode()) testOK = false;

            //test on method compareTo
            if (D.compareTo(DD) != 0) testOK = false;
            compt++;
        }

        System.out.println();
        System.out.println("test result = " + testOK);
    }
}
